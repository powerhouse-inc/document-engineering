...
import { type FlattenedNode, type NodeSortOrder, type NodeSortType, NodeStatus, type SidebarNode } from '../../types.js'
import {
  filterStatuses,
  getMaxDepth,
  getNodePath,
...
interface SidebarContextType {
  ...
  setNodes: (newNodes: SidebarNode[], sortType?: NodeSortType, sortOrder?: NodeSortOrder) => void
...
const SidebarProvider = ({ children, nodes: initialNodes }: SidebarProviderProps) => {
  ...
  const [onActiveNodeChange, setOnActiveNodeChange] = useState<(nodeId: SidebarNode) => void>(() => () => undefined)
  ...
  const currentRoots = useMemo(() => {
    ...
    if (_state.pinnedNodePath.length > 0) {
      roots = _state.pinnedNodePath[_state.pinnedNodePath.length - 1].children ?? []
    }
  ...
  const setActiveNodeChangeCallback = useCallback((callback: (node: SidebarNode) => void) => {
  ...
  const setNodes = useCallback(
    (newNodes: SidebarNode[], sortType?: NodeSortType, sortOrder?: NodeSortOrder) => {
      dispatch({
