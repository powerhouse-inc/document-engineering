 ...
 */
export const useSidebarResize = ({ defaultWidth = 300, minWidth = 100, maxWidth }: SidebarResizeProps) => {
  // Track if the sidebar is currently being resized
  const [isResizing, setIsResizing] = useState<boolean>(false)

  // Current width of the sidebar in pixels
  const [sidebarWidth, setSidebarWidth] = useState<number>(Math.max(defaultWidth, minWidth))
