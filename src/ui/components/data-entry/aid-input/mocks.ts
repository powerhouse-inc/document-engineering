...
const filterOptions = (options: AIDOption[], userInput: string, context?: Record<string, unknown>) => {
  const normalizedInput = userInput.toLowerCase()
  const supportedNetworks: AIDInputProps['supportedNetworks'] = Array.isArray(context?.supportedNetworks)
...
// Sync versions
export const fetchOptionsSync = (userInput: string, context?: Record<string, unknown>): AIDOption[] => {
  return filterOptions(mockedOptions, userInput, context)
}
