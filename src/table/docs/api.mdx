...
- `selectAllRows()`: Select all rows (includes placeholder rows up to `minRowCount`). See [Rows → Row Height and Count](?path=/docs/data-display-object-set-table-docs-rows--readme#row-height-and-count).
...
- `sortRows(columnIndex: number, direction: 'asc' | 'desc' | null): Promise<void>`: Sort by column. Pass `null` to clear sorting.
