...
describe('NumberField', () => {
  ...
  it('should match snapshot', () => {
    const { container } = renderWithForm(
      <NumberField label="Test Label" name="Label" value={345} trailingZeros precision={2} />
    )
    ...
    render(
      <Form onSubmit={mockOnSubmit}>
        {({ formState: { isSubmitting } }) => (
          <div>
            <NumberField label="BigInt Field" name="BigInt Field" numericType="BigInt" />
  ...
  it('should render error messages when provided', () => {
    renderWithForm(
      <NumberField label="Test Label" onChange={mockOnChange} errors={['Error 1', 'Error 2']} name="Label" />
    )
  ...
  it('should render label when label is provided', () => {
    renderWithForm(<NumberField label="Test Label" onChange={mockOnChange} name="Label" />)
    expect(screen.getByLabelText('Test Label')).toBeInTheDocument()
  })

  it('should render the description when provided', () => {
    renderWithForm(
      <NumberField label="Test Label" description="This is a description" onChange={mockOnChange} name="Label" />
    )
  ...
  it('should render warning messages when provided', () => {
    renderWithForm(
      <NumberField label="Test Label" onChange={mockOnChange} warnings={['Warning 1', 'Warning 2']} name="Label" />
    )
  ...
  it('should call onChange when the input value changes', async () => {
    const user = userEvent.setup()
    renderWithForm(<NumberField label="Test Label" name="Label" onChange={mockOnChange} />)
  ...
  it('should disable the input when disabled prop is true', () => {
    renderWithForm(<NumberField label="Test Label" onChange={mockOnChange} disabled name="Label" />)
    const input = screen.getByLabelText('Test Label')
  ...
  it('should show the input as required when required prop is true', () => {
    renderWithForm(<NumberField label="Test Label" onChange={mockOnChange} required name="Label" />)
    expect(screen.getByRole('spinbutton')).toHaveAttribute('required')
  })

  it('should support a defaultValue prop', () => {
    renderWithForm(<NumberField label="Test Label" defaultValue={5} onChange={mockOnChange} name="Label" />)
  ...
  it('should handle NonPositiveFloat numeric type correctly', async () => {
    ...
    render(
      <Form onSubmit={mockOnSubmit}>
        {({ formState: { isSubmitting } }) => (
          <div className="flex w-[400px] flex-col gap-4">
            <NumberField label="Float Field" name="floatField" numericType="NonPositiveFloat" precision={2} />
            <Button type="submit" disabled={isSubmitting}>
              Submit
            </Button>
          </div>
  ...
  it('should handle NegativeInt numeric type correctly', async () => {
    ...
    render(
      <Form onSubmit={mockOnSubmit}>
        {({ formState: { isSubmitting } }) => (
          <div className="flex w-[400px] flex-col gap-4">
            <NumberField label="Negative Int Field" name="negativeField" numericType="NegativeInt" />
            <Button type="submit" disabled={isSubmitting}>
              Submit
            </Button>
          </div>
  ...
  it('should show placeholder when value is empty', async () => {
    ...
    renderWithForm(<NumberField label="Number Field" name="numberField" placeholder={placeholder} />)
  ...
  it('should increment/decrement by specified step using keyboard', async () => {
    ...
    renderWithForm(<NumberField label="Step Field" name="stepField" value={10} step={2} onChange={mockOnChange} />)
  ...
  it('should handle precision correctly for decimal numbers', async () => {
    ...
    render(
      <Form onSubmit={mockOnSubmit}>
        {({ formState: { isSubmitting } }) => (
          <div className="flex w-[400px] flex-col gap-4">
            <NumberField label="Precision Field" name="precisionField" precision={2} />
            <Button type="submit" disabled={isSubmitting}>
              Submit
            </Button>
          </div>
  ...
  it('should accept integer values when numericType is PositiveFloat', async () => {
    ...
    render(
      <Form onSubmit={mockOnSubmit}>
        {({ formState: { isSubmitting } }) => (
          <div>
            <NumberField label="Positive Float Field" name="positiveFloatField" numericType="PositiveFloat" />
            <Button type="submit" disabled={isSubmitting}>
              Submit
            </Button>
          </div>
  ...
  it("should retain the input value '4546-56' and display an error message", async () => {
    ...
    render(
      <Form onSubmit={mockOnSubmit}>
        {({ formState: { isSubmitting } }) => (
          <div>
            <NumberField label="Special Case Field" name="specialCaseField" onChange={mockOnChange} />
            <Button type="submit" disabled={isSubmitting}>
              Submit
            </Button>
          </div>
