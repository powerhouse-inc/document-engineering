...
export const isSupportedErrorObject = (error: object): error is SupportedErrorObject => {
  return typeof error === 'object' && Object.hasOwn(error, 'data') && Array.isArray((error as { data?: unknown }).data)
}

export const defaultOnError = (error: Error, fieldNames: string[]): Record<string, string> => {
