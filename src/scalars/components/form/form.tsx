...
interface FormProps {
   ...
   */
  submissionErrorMatcher?: (error: Error, fieldNames: string[]) => Record<string, string>
...
const Form = forwardRef<UseFormReturn, FormProps>(
  (
    ...
    }, [resetOnSuccessfulSubmit, methods, methods.formState.isSubmitSuccessful, defaultValues])
    ...
    const wrappedOnSubmit = useCallback(
        ...
        try {
          ...
          if (submitChangesOnly && !!defaultValues) {
            ...
            data = Object.fromEntries(
              Object.entries(rawData).filter(([fieldName, value]) => !deepEqual(value, defaultValues[fieldName]))
            )
          }
      ...
      [methods, submitChangesOnly, defaultValues, formId, onSubmit, submissionErrorMatcher]
    )
    ...
    return (
      <FormProvider {...methods}>
        {!renderSubmitErrorsManually && methods.formState.errors.root?.serverError.message && (
          <div className="mb-2">
