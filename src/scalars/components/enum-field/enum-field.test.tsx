...
describe('EnumField Component', () => {
  ...
  it('should match snapshot with Select variant', () => {
    const { asFragment } = renderWithForm(<EnumField {...defaultProps} variant="Select" />)
    expect(asFragment()).toMatchSnapshot()
  })
  ...
  it('should render description when provided', () => {
    renderWithForm(<EnumField {...defaultProps} variant="Select" description="Test description" />)
    expect(screen.getByText('Test description')).toBeInTheDocument()
  })

  it('should render warning messages when provided', () => {
    renderWithForm(<EnumField {...defaultProps} warnings={['Warning message']} />)
  ...
  it('should handle value changes in RadioGroup variant', async () => {
    ...
    renderWithForm(<EnumField {...defaultProps} value="option1" onChange={onChange} />)
  ...
  it('should handle value changes in Select variant', async () => {
    const user = userEvent.setup()
    const onChange = vi.fn()
    renderWithForm(<EnumField {...defaultProps} variant="Select" multiple value={['option1']} onChange={onChange} />)
