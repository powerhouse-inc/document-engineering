...
describe('UrlField', () => {
  ...
  it('should render with basic props', () => {
    renderWithForm(
      <UrlField data-testid="url-field" name="test-url" label="Website URL" placeholder="https://example.com" />
    )
  ...
  it('should show required indicator when required', () => {
    renderWithForm(<UrlField data-testid="url-field" name="test-url" label="Website URL" required />)
  ...
  it('should show description when provided', () => {
    renderWithForm(<UrlField name="test-url" label="Website URL" description="Enter your website URL" />)
  ...
  it('should show warning message when provided', () => {
    renderWithForm(<UrlField name="test-url" label="Website URL" warnings={['URL may be unreachable']} />)

    expect(screen.getByText('URL may be unreachable')).toBeInTheDocument()
  })

  it('should be disabled when disabled prop is true', () => {
    renderWithForm(<UrlField data-testid="url-field" name="test-url" label="Website URL" disabled />)
  ...
  it('should validate URL format', async () => {
    ...
    expect(await screen.findByText('not-a-url must be a valid URL')).toBeInTheDocument()
  })

  it('should be invalid if URL is to long', async () => {
    const user = userEvent.setup()
    renderWithForm(<UrlField data-testid="url-field" name="test-url" label="Website URL" maxURLLength={10} />)

    const input = screen.getByTestId('url-field')
    await user.type(input, 'https://example.com/{enter}') // 20 characters (including protocol and domain)

    expect(input).toHaveAttribute('aria-invalid', 'true')
    expect(await screen.findByText('Website URL must not exceed 10 characters')).toBeInTheDocument()
  })

  it('should show warning when the URL could be truncated', async () => {
    const user = userEvent.setup()
    renderWithForm(<UrlField data-testid="url-field" name="test-url" label="Website URL" />)
  ...
  it('should not show warnings if showWarnings prop is false', async () => {
    const user = userEvent.setup()
    renderWithForm(<UrlField data-testid="url-field" name="test-url" label="Website URL" showWarnings={false} />)
