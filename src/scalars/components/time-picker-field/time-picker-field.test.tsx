...
describe('TimePickerField', () => {
  it('should match the snapshot', () => {
    const { container } = renderWithForm(
      <TimePickerField label="Test Label" name="test-time" id="test-id" required disabled={false} />
    )
  ...
  it('should mark the label as required when required prop is true', () => {
    renderWithForm(<TimePickerField name="test-time" label="Test Label" required />)
  ...
  it('should mark the label as disabled when disabled prop is true', () => {
    renderWithForm(<TimePickerField name="test-time" label="Test Label" disabled />)
    const label = screen.getByText('Test Label')
  ...
  it('should handle timeIntervals prop correctly with 15-minute intervals', async () => {
    ...
    renderWithForm(<TimePickerField name="test-time" label="Test Label" timeIntervals={15} />)
  ...
  it('should handle timeIntervals prop correctly with 30-minute intervals', async () => {
    const user = userEvent.setup()
    renderWithForm(<TimePickerField name="test-time" label="Test Label" timeIntervals={30} />)
  ...
  it('should set timezone value and disable select when timeZone prop is provided', async () => {
    ...
    renderWithForm(<TimePickerField name="test-time" label="Test Label" timeZone={timeZoneValue} />)
  ...
  it('should display continent when includeContinent prop is true', async () => {
    ...
    renderWithForm(<TimePickerField name="test-time" label="Test Label" timeZone={timeZoneValue} includeContinent />)
  ...
  it('should not display continent when includeContinent prop is false', async () => {
    ...
    renderWithForm(
      <TimePickerField name="test-time" label="Test Label" timeZone={timeZoneValue} includeContinent={false} />
    )
