...
export interface FormLabelProps extends React.PropsWithChildren, React.LabelHTMLAttributes<HTMLLabelElement> {
  required?: boolean
  disabled?: boolean
  description?: string
  hasError?: boolean
...
export const FormLabel: React.FC<FormLabelProps> = ({
  ...
  const classes = cn(
    ...
    disabled && `cursor-not-allowed text-gray-700 ${inline ? 'dark:text-gray-600' : 'dark:text-gray-300'}`,
    inline ? !disabled && 'group-hover:text-gray-900 dark:group-hover:text-slate-50' : 'mb-[3px]',
    className
  )
      ...
      {required && (
        <span
          className={cn(
            ...
            hasError && `${inline ? '!text-red-800' : '!text-red-900'} group-hover:!text-red-900`
          )}
        >
          *
        </span>
