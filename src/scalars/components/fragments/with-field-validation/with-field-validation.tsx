...
export const withFieldValidation = <T extends PossibleProps, R extends React.ElementRef<any> = React.ElementRef<any>>(
  ...
  const ComponentWithFieldValidation = React.forwardRef<R, T>(
    ({ value, name, showErrorOnBlur, showErrorOnChange, validators: customValidators, ...props }, ref) => {
      ...
      const errors = [...(formErrors[name]?.types ? (Object.values(formErrors[name].types ?? []) as string[]) : [])]
      ...
      if (value !== undefined && !onChangeProp && process.env.NODE_ENV === 'development') {
        console.warn(
        ...
        <Controller
          ...
          render={({
            ...
            const onChangeCallback = useCallback(
              (event: React.ChangeEvent<HTMLInputElement>) => {
                ...
                if (showErrorOnChange === undefined && showErrorOnBlur === undefined) {
          ...
          rules={{
            ...
            validate: {
              ...
              ...(props.errors
                ? Object.fromEntries(
                    Array.isArray(props.errors)
                      ? props.errors.map((error, index) => [`_propError${index}`, () => error])
                      : []
                  )
                : {}),
              ...
              ...(options?.validations
                ? Object.fromEntries(
                    Object.entries(options.validations).map(([key, validatorFactory]) => {
                      ...
                      return [key, validatorFactory(propsWithValues as unknown as T)]
                    })
                  )
                : {}),
              ...
              ...(customValidators !== undefined
                ? Object.fromEntries(
                    (Array.isArray(customValidators) ? customValidators : [customValidators]).map(
                      (validator, index) => [`customValidation${index}`, validator]
                    )
                  )
                : {}),
            },
