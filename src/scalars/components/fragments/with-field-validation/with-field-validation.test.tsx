...
const TextFieldTesting = React.forwardRef<HTMLInputElement, Omit<TextFieldProps, 'autoComplete'>>(
  ({ label, value, defaultValue, onChange, errors, pattern, ...props }, ref) => {
    return (
      <FormGroup>
        {label && (
          <FormLabel htmlFor="test" required={props.required} disabled={props.disabled} hasError={!!errors?.length}>
            {label}
          </FormLabel>
        )}
        <Input
          ...
          {...props}
        />
        {errors && <FormMessageList data-testid="error-message" messages={errors} type="error" />}
      </FormGroup>
    )
  }
)
...
describe('withFieldValidation', () => {
  ...
  it('should handle multiple extra component validators', async () => {
    ...
    renderWithForm(<WrappedComponent name="test" validators={[validator1, validator2]} />)
  ...
  it('should pass through custom error messages', async () => {
    await act(() => renderWithForm(<WrappedComponent name="test" errors={['Custom error']} />))
