  ...
  if (typeof customIcon === 'string') {
    return (
      <Icon
        ...
        className={cn(
          'shrink-0',
          asPlaceholder ? 'text-gray-400 dark:text-gray-700' : 'text-gray-900 dark:text-gray-300'
        )}
      />
    )
  }
...
const ReloadButton: React.FC<{
  ...
  value: string
}> = ({ isLoadingSelectedOption, handleFetchSelectedOption, isFetchSelectedOptionSync, value }) => (
  <div>
    <button
      type="button"
...
const IdAutocompleteListOption: React.FC<IdAutocompleteListOptionProps> = ({
    ...
    <div className={cn('flex w-full items-center')}>
      <span
        className={cn(
          ...
          asPlaceholder ? 'text-gray-400 dark:text-gray-700' : 'text-gray-500 dark:text-gray-600'
        )}
      >
        {value}
      </span>
  ...
  const renderWithValueTitleAndDescription = (showDescription = true) => (
    <div className={cn('flex w-full flex-col gap-1')}>
      <div className={cn('flex gap-2')}>
        <IconRenderer customIcon={asPlaceholder ? placeholderIcon : icon} asPlaceholder={asPlaceholder} />
        <div className={cn('flex min-w-0 grow flex-col gap-[-2px]')}>
          ...
          {!showValue && typeof path === 'object' ? (
            <a
              ...
              className={cn('truncate text-xs leading-5 text-blue-900 hover:underline focus-visible:outline-none')}
            >
              {path.text}
            </a>
          ) : (
      ...
      {showDescription && 'agentType' in extraProps && typeof extraProps.agentType === 'string' && (
  ...
  return (
    ...
    >
      ...
      {variant === 'withValueTitleAndDescription' && renderWithValueTitleAndDescription()}
    </div>
  )
}
